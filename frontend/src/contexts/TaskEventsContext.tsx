...
interface TaskEventsContextValue {
  trackMutation: (clientMutationId: string) => void
}
...
/**
 * Provider that manages a single SSE connection for task events.
 * Should be placed at the app level to ensure only one connection exists.
 */
export function TaskEventsProvider({ children, enabled = true }: TaskEventsProviderProps) {
  ...
  return (
    <TaskEventsContext.Provider value={{ trackMutation }}>
      {children}
...
/**
 * Hook to access the task events context.
 * Returns the trackMutation function for deduplication.
 */
